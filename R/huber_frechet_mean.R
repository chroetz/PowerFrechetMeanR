#' Compute the Huber Frechet Mean
#'
#' Given \eqn{n} data points \eqn{x_1, \dots, x_n \in \mathbb{R}^d} and one
#' or more threshold parameters \eqn{\delta > 0}, the **Huber Frechet mean**
#' for each \eqn{\delta} is the minimiser of
#' \deqn{f(x) = \sum_{i=1}^{n} \rho_\delta(\|x_i - x\|),
#'   \quad x \in \mathbb{R}^d,}
#' where the Huber loss is
#' \deqn{\rho_\delta(r) = \begin{cases} r^2/2 & r \le \delta \\ \delta r -
#'   \delta^2/2 & r > \delta. \end{cases}}
#'
#' For large \eqn{\delta} all distances satisfy \eqn{r_i \le \delta}, so the
#' objective reduces to \eqn{\tfrac{1}{2}\sum \|x_i - x\|^2} and the minimiser
#' is the arithmetic mean.  For small \eqn{\delta} the loss is approximately
#' linear (\eqn{\delta \|x_i - x\|}) and the minimiser approaches the
#' geometric median.
#'
#' ## Warm-start strategy
#'
...
#' ## Optimisation
#'
#' Uses an analytical gradient (C++/RcppArmadillo) with no smoothing constant
#' (the Huber gradient has no singularity) and L-BFGS-B from [stats::optim()].
#'
#' @param X      Numeric matrix of data points, one row per observation
#'   (\eqn{n \times d}).  A numeric vector is treated as an \eqn{n \times 1}
#'   matrix.
#' @param delta  Numeric vector of Huber thresholds, each \eqn{> 0}.
...
#'   \describe{
#'     \item{`delta`}{Huber threshold.}
#'     \item{`mean`}{Matrix column (\eqn{k \times d}); row \eqn{i} is the
#'       Huber Frechet mean for `delta[i]`.  Access as `res$mean[i, ]`.}
...
#' res_multi <- huber_frechet_mean(X, delta = c(0.5, 1, 2, 5))
#' res_multi
#'
#' @seealso [huber_objective_value()] to evaluate the objective at an arbitrary
#'   point; [pseudo_huber_frechet_mean()] for the smooth pseudo-Huber variant.
#' @export
huber_frechet_mean <- function(X,
                               delta,
                               tol   = 1e-10,
                               maxit = 1000L,
                               ...) {
  ...
  for (i in seq_along(delta_vals)) {
    ...
    fit <- stats::optim(
      par     = init,
      fn      = function(x) huber_objective_cpp(x, X, d_i),
      gr      = function(x) huber_gradient_cpp(x, X, d_i),
      method  = "L-BFGS-B",
      control = ctrl
    )
...
#' Evaluate the Huber Frechet Objective at a Given Point
#'
#' Evaluates \eqn{f(x) = \sum_{i=1}^{n} \rho_\delta(\|x_i - x\|)} at an
#' arbitrary point `x`, where \eqn{\rho_\delta} is the Huber loss with
#' threshold \eqn{\delta}.
#'
#' @param x      Numeric vector of length \eqn{d}.
#' @param X      Numeric \eqn{n \times d} matrix of data points.
#' @param delta  Huber threshold \eqn{\delta > 0}.
#'
#' @return Scalar numeric objective value.
#'
#' @examples
#' X <- matrix(1:6, nrow = 3)
#' huber_objective_value(c(2, 3), X, delta = 1)
#'
#' @seealso [huber_frechet_mean()]
#' @export
huber_objective_value <- function(x, X, delta) {
  if (is.vector(X) && is.numeric(X)) X <- matrix(X, ncol = 1L)
  X <- as.matrix(X)
  storage.mode(X) <- "double"
  huber_objective_cpp(as.numeric(x), X, delta)
}
